    ...
    effects: [
      ...
      },
      {
        ...
        entry: {
          ...
          subjectIds: [charA.id, charB.id],
          perspective: "target",
          triggerTags: ["conflict", "humiliated"],
          unresolved: true,
          emotions: { shame: 40 },
        },
      },
...
function gossipEmpressScold(db: ContentDB, state: GameState, seed: string): GossipPlan | null {
  ...
  const empressId = activeEmpressId(state) ?? "shen_zhibai";
