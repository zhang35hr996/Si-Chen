...
export const gameStateSchema = z.strictObject({
  ...
  haremInvestigationLeads: z.record(z.string().regex(/^ilead_\d{6}$/), z.strictObject({
    ...
    sourceEvidenceNodeId: z.string().optional(),
  ...
  investigationPublicReports: z.array(z.union([
    // 皇嗣异常报告（anomaly）
    z.strictObject({
      id: z.string().min(1),
      source: z.strictObject({ kind: z.literal("investigation_incident"), incidentId: z.string().min(1) }),
    ...
    // 证据驱动调查进展通报（investigation_update / investigation_final）
    z.strictObject({
      id: z.string().min(1),
      source: z.strictObject({ kind: z.literal("investigation_incident"), incidentId: z.string().min(1) }),
