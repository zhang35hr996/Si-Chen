/**
 * 紫宸殿奏折框架（Phase 4A 第一刀）。通用 Memorial 模型 + **地方灾情**范例：确定性生成、原子批阅、后果一律
 * 经正式 effect funnel（绝不直接改 state）。其余类别（treasury/military/justice）仅占位，人事奏折仍由
 * PR3C-3b 的 personnelDecisions 承载（未并入本框架）。
 */
...
import type { DisasterOption, GameState, Memorial, MemorialResourceEffect } from "../state/types";
...
/** 灾情三选项（确定性，按严重度缩放；仅 funnel 可寻址的 resource effect，受 AXIS_CAP 限）。 */
function disasterOptions(severity: "minor" | "major"): DisasterOption[] {
  const m = severity === "major" ? 2 : 1;
  return [
    { id: "relief", label: "开仓赈济", effects: [res("nation", "publicSupport", 4 * m), res("nation", "clanDiscontent", -3 * m), res("nation", "productivity", 2 * m)] },
    { id: "tax_remit", label: "蠲免赋税", effects: [res("nation", "publicSupport", 3 * m), res("nation", "clanDiscontent", -4 * m), res("nation", "productivity", -2 * m)] },
    { id: "ignore", label: "不予理会", effects: [res("nation", "publicSupport", -4 * m), res("nation", "rumor", 3 * m), res("sovereign", "regimeSecurity", -3 * m)] },
  ];
}
...
/**
 * 原子批阅一条奏折：验证 pending → 验证 optionId 合法 → 经 funnel 施加该选项后果 → 标记 resolved。任一步失败
 * 返回 err，输入 state 不变（不施后果、不标 resolved）。绝不先 resolve 再执行后果。
 */
export function resolveMemorial(
  ...
  if (m.status !== "pending") return err(stateError("MEMORIAL_ALREADY_RESOLVED", `奏折「${memId}」已批阅`, { context: { memId, status: m.status } }));

  // 第一刀只实现 disaster；其余 category 无 payload 变体（schema 已排除）。
  const option = m.payload.options.find((o) => o.id === optionId);
  if (!option) return err(stateError("MEMORIAL_BAD_OPTION", `选项「${optionId}」不属于奏折「${memId}」`, { context: { memId, optionId } }));

  // option.effects 形状即 resource EventEffect 子集；funnel 的 validateEffects 仍会校验 field 合法性。
  const applied = applyEffects(db, state, option.effects as EventEffect[], { sceneId: "memorial" });
  if (!applied.ok) return err(applied.error[0]!);

  const resolved: Memorial = { ...m, status: "resolved", resolvedAt: at, resolution: optionId };
  return ok({ state: { ...applied.value, memorials: { ...applied.value.memorials, [m.id]: resolved } } });
}
...
/** 奏折集合完整性校验（key/去重/类别一致/状态-裁断一致/disaster 载荷）。纯函数，供 load 与测试复用。 */
export function validateMemorials(state: GameState): GameError[] {
  ...
  for (const [key, m] of Object.entries(state.memorials)) {
    if (m.id !== key) e("MEMORIAL_KEY_MISMATCH", `memorials["${key}"].id = "${m.id}"（键不一致）`, { key, id: m.id });
    if (seenSource.has(m.sourceId)) e("MEMORIAL_DUP_SOURCE", `奏折来源「${m.sourceId}」重复`, { id: m.id, sourceId: m.sourceId });
    seenSource.add(m.sourceId);
    if (m.category !== m.payload.category) e("MEMORIAL_CATEGORY_MISMATCH", `奏折「${m.id}」category「${m.category}」≠ payload「${m.payload.category}」`, { id: m.id });

    // disaster 载荷：地域已知、选项非空、optionId 唯一。
    if (m.payload.category === "disaster") {
      if (!DISASTER_REGIONS[m.payload.regionId]) e("MEMORIAL_BAD_REGION", `奏折「${m.id}」regionId「${m.payload.regionId}」非已知地域`, { id: m.id });
      const ids = new Set<string>();
      if (m.payload.options.length === 0) e("MEMORIAL_NO_OPTIONS", `奏折「${m.id}」无可选项`, { id: m.id });
      for (const o of m.payload.options) {
        if (ids.has(o.id)) e("MEMORIAL_DUP_OPTION", `奏折「${m.id}」选项 id「${o.id}」重复`, { id: m.id });
        ids.add(o.id);
      }
    }
    ...
    if (m.status === "pending") {
      if (m.resolvedAt !== undefined || m.resolution !== undefined) e("MEMORIAL_PENDING_WITH_RESOLUTION", `待批奏折「${m.id}」不应带 resolvedAt/resolution`, { id: m.id });
    } else {
      if (m.resolvedAt === undefined || m.resolution === undefined) {
        ...
        if (!m.payload.options.some((o) => o.id === m.resolution)) e("MEMORIAL_BAD_RESOLUTION", `奏折「${m.id}」resolution「${m.resolution}」非合法选项`, { id: m.id });
        if (compareGameTime(m.resolvedAt, m.createdAt) < 0) e("MEMORIAL_RESOLVED_BEFORE_CREATED", `奏折「${m.id}」resolvedAt 早于 createdAt`, { id: m.id });
      }
    }
  }
  return errors;
