...
中文古风宫廷叙事模拟游戏。玩家是礼法女尊帝国的皇帝，在江山、后宫、血脉三柱之间经营，与拥有**独立主观记忆**的角色对话。对话未来由 AI 生成、引擎管控；当前骨架阶段全部为脚本内容。

> 世界观权威参考：[`docs/background-v1.0.md`](docs/background-v1.0.md)（位分 / 称谓 / 属性系统的唯一权威）· 文档总索引：[`docs/README.md`](docs/README.md) · 引擎实现契约：[`docs/engineering/10-current-implementation.md`](docs/engineering/10-current-implementation.md)
>
...
| `npm run validate-content` | 校验 `content/` 全部 JSON（schema + 交叉引用 + 场景图），坏内容退出非零并列出所有错误 |
...
新游戏开局：**元年一月上旬**，落在 **皇城主地图（宫城图）**——地图即主界面，
事件结束后也自动跳回此处。点地图上的地点节点进入该地点（当前所在免行动点、
其余快速旅行）；进入后可触发该地点事件，事件结束回到主地图。

- **地点**（地图节点；宫城图主图 + 后宫/京城/郊外子图）
  - 主图（前朝）：紫宸殿（书房）· 宣政殿（朝会）· 文昭殿（皇嗣开蒙）· 毓庆宫（未成年皇嗣居所）· 奉先殿（择养父，祭祀）· 慈宁宫（太后住所）· 御花园 · 后宫
  - 后宫子图：坤宁宫 · 咸福宫 · 昭宁宫 · 承晖宫 · 景仁宫 · 钟粹宫 · 延和宫 · 霁月宫 · 储秀宫（新增七宫暂共用背景）
- **角色**（各在自己的地点，卡片按身份分流渲染：侍君显示位分 + 属性 + 恩宠，官员显示官职）：
  - 沈知白（凤后·正宫）
  - 徐清欢（从一品·君）
  - 陆怀瑾（正三品·承徽，居钟粹宫，测试失宠/情绪线）
  - 温雅（长门宫·冷宫旧人）
  - 卫绥（正五品官员，证明系统不默认人人是侍君）
- **子嗣 / 毓庆宫**：皇嗣全生命周期（受孕→诞育→小名→百日宴正名→开蒙→文昭殿教育→奉先殿择养父）；皇子(女)满 5、皇郎(男)满 7 迁居毓庆宫，可在毓庆宫或紫宸殿子嗣名册召见（1 行动点 +恩宠）
- **上朝会话（宣政殿）**：每日卯时（首个行动点）可上朝；进殿即扣 **1 行动点**，整场只此一次。系统从 10 件朝政事务中**随机抽取 2–3 件**，逐件禀报、逐件裁决（一件处理完再报下一件，互不混杂）。HUD 设 **「退朝」**：随时可退，已裁之事照准入账、余事作罢，直接回紫禁城主图（已扣的 1 点不退）。上朝/退朝后主图上的「可上朝」标注当日即消失，次日卯时再现
- **位分升降 + 封号**：可对侍君（除凤后外）升/降位分、加/褫夺封号（0 行动点）；操作后侍君即时反应（谢恩/请罪/惶恐）；称呼 = 封号（若有）或姓 + 当前位分
...
每句台词——无论 MockProvider 回显还是将来 LLM 生成——都过同一组**纯文本门**（`engine/dialogue/gates.ts`）：禁用词扫描（含「女官」「女帝」等）、自称越位（用了别的位分的自称）、皇帝称谓越位（一律「陛下」）、prompt 模板 token 泄漏。命中 reject 即拒绝该行，命中 flag 则照常呈现但标记 degraded；全部写入诊断日志。**数值/状态校验不在这里**，归效果漏斗（`engine/effects/`）——文本问题与状态问题由两层独立兜住。远程 provider 适配器**只有骨架**（类型 + `createRemoteProvider` 桩，永远返回 `NOT_CONFIGURED`，零网络零密钥，运行时不 import）。
...
- checksum 只是完整性/篡改检测，不是密码学安全、更不是防作弊。目的只是抓坏 JSON、手滑误改、写入截断。实现是同步的 64 位 FNV-1a（用 `crypto.subtle` 会把整条存档路径逼成 async），相对计划里的 sha-256 是有意偏差。
- **内容版本不一致分两档，绝不静默载入：**
...
逻辑 **key** 在 `assets/manifest.json`，文件本体在 `public/assets/`，代码/内容只认 key。**当前已全部是真图**（`validate-manifest` 报 100% real art）；对话/地点是 galgame 式布局（背景铺满 + 立绘居中靠下 + 底部对话框），地图屏用皇宫俯视图作底图。标题屏以 `bg.game_start` 开篇背景垫底、上覆暗纹光晕，「凤司晨」居中其上。

**换图**：同名覆盖文件即可，key 不变。放完跑 `npm run validate-manifest`。
**立绘建议用竖图 + 透明背景 PNG**（约 2:3，如 640×960+）
**想给某角色单独立绘或多表情**：把该角色 `portraitSet` 改成独有名（如 `shen_zhibai`）、在 `expressions` 里列出表情、在 manifest 加 `portrait.<set>.<表情>` 条目并放文件——场景里写 `expression: "smile"` 就会生效（缺图时自动回退 neutral，绝不崩）。
...
tests/                   # Vitest 单元/集成（476 用例）+ tests/e2e Playwright 冒烟；出货的 content/ 本身就是 fixture
...
4. **Scaffold 守卫**：三柱资源（民心/军力等）可被效果修改、可在 debug 看、随存档走，但**不能**被触发条件/分支/未来的 prompt 读取——条件 DSL 结构上没有资源谓词。
