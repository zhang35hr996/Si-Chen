...
| `src/engine/characters/presence.ts`（改） | 加 `consortLocationAt`、`presentAt`、`absentAt`（不改既有两函数） |
| `src/engine/characters/gongli.ts`（新） | `attendantsOf`、`reportingAttendant`（宫隶确定性派生） |
| `assets/manifest.json`（改） | 加 6 条 `portrait.gongliN.neutral` |
...
## Task 6: 宫隶生成（确定性派生 + manifest）

**Files:**
- Create: `src/engine/characters/gongli.ts`
- Modify: `assets/manifest.json`
- Test: `tests/characters/gongli.test.ts`

**Interfaces:**
- Consumes: `fnv1a64Hex`（`../save/canonical`）；`MALE_ATTENDANT_RESERVED_CHARS`（`./lilangNames`）。
- Produces:
  - `GongliAttendant { name: string; portraitSet: string }`
  - `attendantsOf(rngSeed: number, consortId: string): [GongliAttendant, GongliAttendant]`
  - `reportingAttendant(rngSeed: number, consortId: string, dayIndex: number): GongliAttendant`

- [ ] **Step 1: 写失败测试**

创建 `tests/characters/gongli.test.ts`：
...
Run: `npx vitest run tests/characters/gongli.test.ts`
Expected: FAIL — 模块/函数不存在。

- [ ] **Step 3: 实现 `src/engine/characters/gongli.ts`**
...
- [ ] **Step 5: manifest 加 6 条 gongli 立绘**

在 `assets/manifest.json` 的 `entries` 内（紧邻其它 `portrait.*` 条目，如 `portrait.cheng_feng.neutral` 之后）加：

```json
    "portrait.gongli1.neutral": { "path": "portraits/gongli/gongli1.png", "kind": "portrait", "placeholder": false },
    "portrait.gongli2.neutral": { "path": "portraits/gongli/gongli2.png", "kind": "portrait", "placeholder": false },
    "portrait.gongli3.neutral": { "path": "portraits/gongli/gongli3.png", "kind": "portrait", "placeholder": false },
    "portrait.gongli4.neutral": { "path": "portraits/gongli/gongli4.png", "kind": "portrait", "placeholder": false },
    "portrait.gongli5.neutral": { "path": "portraits/gongli/gongli5.png", "kind": "portrait", "placeholder": false },
    "portrait.gongli6.neutral": { "path": "portraits/gongli/gongli6.png", "kind": "portrait", "placeholder": false },
```

注意：上一条目尾部记得补英文逗号，保持 JSON 合法。

- [ ] **Step 6: 校验 manifest**

Run: `npm run validate-manifest`
Expected: 通过（6 个 gongli 文件存在于 `public/assets/portraits/gongli/`，路径有效）。

- [ ] **Step 7: 提交**

```bash
npx tsc --noEmit
git add src/engine/characters/gongli.ts assets/manifest.json tests/characters/gongli.test.ts
git commit -m "feat: 宫隶确定性派生（名字/立绘/禀告者）+ manifest gongli 立绘"
```

---

## Task 7: 缺席禀报（pure `absentAt` + 宫隶口吻 + CharacterScene 渲染）
...
**Interfaces:**
- Consumes: `presentAt`、`getPresentAt`、`consortLocationAt`（presence.ts）；`resolveDisplayName`（standing.ts）；`reportingAttendant`、`GongliAttendant`（Task 6 `gongli.ts`）。
- Produces:
  - `absentAt(db, state, locationId): Record<string, string>` —— 住客中此刻不在该地者 → 其当前所在 locationId。
  - `CharacterScene` 新增可选 prop `absence?: Record<string, string | undefined>`；住客缺席时改用**禀告宫隶的立绘 + 名牌 + 口吻台词**，屏蔽对话/侍寝。
...
- [ ] **Step 6: CharacterScene 接受 absence、由宫隶口吻+立绘禀报**
...
- 在 `const standing = ...` 之后加去向 + 禀告宫隶 + 台词：
...
- **立绘**：缺席时显示宫隶立绘而非侍君空位。在 `char-scene__sprite-wrap` 的 `character ? (<img ... 侍君立绘/>) : (<空位/>)` 中，把侍君 `<img>` 改成「有宫隶禀告→宫隶立绘；否则→侍君立绘」。即把：
...
- **名牌 + 台词**：在 `char-scene__dialogue` 的 `character` 真分支里，缺席时名牌显示「{宫隶名}·{侍君}的宫人」并以禀报取代问候与动作坞。把：
...
（即把「问候行 + `action-dock` 整块」包进 `awayLine ? <禀报> : <>...</>`；缺席只显宫隶禀报、无对话/侍寝/查看详情/更多按钮。`npx tsc --noEmit` 校验括号配对。）

- [ ] **Step 7: 类型检查 + 构建 + 手动验证**

Run: `npx tsc --noEmit && npm run build`
手测：`npm run dev`，卯时（新开局即卯时）进任一后宫居所（如重萃宫）→ 应显示一名宫隶立绘 + 名牌（如「听雪 · 陆承徽的宫人」）+「听雪垂手禀道：『陆承徽往坤宁宫向皇后请安去了。』」，且无侍寝/对话按钮；花 1 AP 到辰时再进 → 住客回来、恢复侍君立绘/问候/按钮。
Expected: 符合预期。
...
git commit -m "feat: 后宫居所缺席禀报（absentAt + 宫隶口吻立绘）"
```

---

## Task 8: 卯时坤宁宫 乘风请安遮罩（进入主殿/退出）
...
**Interfaces:**
- Consumes: `ceremonyOpen` 状态、`setCeremonyOpen`（Task 8）。
...
- **Spec 覆盖**：§4 临场→T1–3；§5 缺席禀报→T7；§5.1 宫隶→T6；§6 请安仪式→T8–9；§7 免请安→T4–5、T10；§8 状态字段→T1；§9 测试散落各 TDD 步；§10 非目标未触碰。
- **占位符**：无 TBD/TODO；每步含真实代码/命令/期望。
- **类型一致**：`excusedFromGreeting`/`overnightWith` 形状、`consortLocationAt`/`presentAt`/`absentAt`/`greetingAttendees`/`attendantsOf`/`reportingAttendant`/`excuseFromGreeting`/`recordOvernight` 签名跨任务一致。
- **已知风险**：(a) `greeting.ts`↔`presence.ts` 循环 import——T8 Step3 备注了迁移退路；(b) UI 任务的 App 变量名（`goHome`/`liveState`/LocationScreen 渲染处 props）以文件实际写法对齐，计划已注明「按既有写法」。
