# 健康 / 病情 / 生死 系统 Phase 3 Implementation Plan v3.1（太医 · 怀孕成本 · 重病/服丧 gating）
...
- **看诊文案显示实际回血**：UI 用 `actualHealing = outcome.nextHealth − outcome.previousHealth`（而非 `rolledHealing`，因 clamp 可能使实际 < 掷出值）。`actualHealing > 0` → 「调理一番，气色稍复（健康 +N）」；治愈病情（`cured`）→ 「药石见效，病气已退」；既未治愈又 `actualHealing === 0` → 「太医请陛下继续静养」（**不显示虚假 +0**）。治愈且有回血可两句并陈。
...
- **生产成本映射**（照抄）：`bearerOutcome === "safe"` → 顺产 `−5`；`bearerOutcome === "child_dies"` → 难产 `−10`（母存活）；`bearer_dies` / `both` → **不追加健康成本**（母方已由 birth 效果置 `deceased`）。
...
- `src/engine/effects/funnel.ts` — `record_physician_visit` 的 validate（合法性强制）+ apply case；`taihou_decease` apply 增写 `mourningUntilDayExclusive`。
...
- `src/ui/components/PhysicianModal.tsx` — 看诊区常驻 + 流胎/承养区可共存（消除互斥分支）。
- `src/ui/App.tsx` — 看诊 handler（reaction，actualHealing 文案）；gating 接入；转胎 −10 接线。
...
>   {!selfCarrying && consortCarrying && <TransferredPregnancyNotice />}
> </>
> ```
> **删除**旧版「selfCarrying 时只显示流胎 / 三个互斥 ternary」的全部文字与实现。
...
function freshStore() {
  return new GameStore(createNewGameState(db));
}
...
describe("看诊事务（resolveTimedAction 整笔回滚）", () => {
  it("第一次看诊：加血、记录月键、扣 1 AP", () => {
    ...
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(r.value.calendar.ap).toBe(apBefore - 1);
    expect(r.value.resources.sovereign.health).toBe(hpBefore + plan.actualHealing);
    expect(r.value.resources.sovereign.lastPhysicianVisitMonthKey).toBe(`${at.year}:${at.month}`);
  });
  ...
  it("同月对同一目标第二次（非法效果）：整笔失败，health/AP/calendar 不变", () => {
    ...
    const illegal = [
      ...
      { type: "record_physician_visit", subject: { kind: "sovereign" }, monthKey: `${at.year}:${at.month}` } as const,
    ];
    const r2 = store.resolveTimedAction(db, illegal, { type: "SPEND_AP", amount: 1 });
    expect(r2.ok).toBe(false); // record validate 拒绝（本月已请脉）
    expect(store.getState().resources.sovereign.health).toBe(snapshot.resources.sovereign.health); // 未加 9
    expect(store.getState().calendar).toEqual(snapshot.calendar); // AP/时间未动
  });
...
> 先读 `src/store/gameStore.ts` 确认 `GameStore` 构造签名与 `resolveTimedAction` 返回 `Result`，照实对齐（构造可能是 `new GameStore(state)` 或工厂；命令形状可能不是 `{ type: "SPEND_AP" }`——按现状改）。
...
const doConsult = (subject: PhysicianSubject) => {
  ...
  if (lines.length === 0) lines.push("太医诊脉施治，请陛下继续静养。");
...
### Task 7: 月度承养健康成本接线（`pregnancyMonthlyCost`）

**Files:**
- Modify: `src/store/healthTick.ts:109-128`（侍君遍历传 `pregnancyMonthlyCost`）
...
describe("月度承养健康成本", () => {
  it("pregnancyMonthlyCost=true 比 false 扣得多（同 seed）", () => {
    const base = { health: 80, status: "healthy" as const, age: 25, isYearStart: false, seedKey: "tick:1:c1:1:1" };
    const withPreg = projectMonthlyHealth({ ...base, pregnancyMonthlyCost: true });
    const withoutPreg = projectMonthlyHealth({ ...base, pregnancyMonthlyCost: false });
    expect(withPreg.nextHealth).toBeLessThanOrEqual(withoutPreg.nextHealth);
  });

  it("buildMonthlyHealthTick 对承孕侍君投影成本（delta ≤ 0）", () => {
    ...
    s.resources.bloodline.gestations.push({ carrier: cid, conceivedAt: { year: 1, month: 1, period: "early", dayIndex: 0 }, fatherId: cid, transferredAtMonth: 1 });
    const tick = buildMonthlyHealthTick(db, s);
    const fx = tick.effects.find((e) => e.type === "set_consort_health" && (e as { char: string }).char === cid) as { healthDelta?: number } | undefined;
    if (fx?.healthDelta !== undefined) expect(fx.healthDelta).toBeLessThanOrEqual(0);
  });
});
```
...
Expected: FAIL（orchestrator 仍硬编码 `pregnancyMonthlyCost: false`）

- [ ] **Step 3: 接线 carrier 检测**

`src/store/healthTick.ts` 侍君遍历（109–128 行）：在 `const status = ...` 后加 `const carrying = state.resources.bloodline.gestations.some((g) => g.carrier === consortId);`，并把该处 `projectMonthlyHealth({ ..., pregnancyMonthlyCost: false, seedKey })` 改为 `pregnancyMonthlyCost: carrying`。
...
### Task 9: 生产健康成本 −5/−10（接入 `buildBirth`，固定 seed 无条件断言）
...
- Produces: `buildBirth` 返回 `effects` 在 `birth` 效果**之后**追加母方成本（仅非自孕、母方生还时）；扣血致死则皇嗣已落库（已产存嗣仅母亡）。

> **测试不得用条件断言**：先用一段临时脚本逐 seed 扫描，找出使 `buildBirth` 产出 `bearerOutcome === "safe"` 与 `=== "child_dies"` 的具体 `rngSeed`（造态时设 `s.rngSeed = <found>`），然后**无条件** `expect(plan.bearerOutcome).toBe("safe"/"child_dies")`。先读 `gestation.ts` 的 verdict 裁决逻辑确认 seedKey 构成（依赖哪些字段），固定这些字段后扫 `rngSeed`。

- [ ] **Step 0: 扫描确定 seed（实现期，临时脚本不提交）**

写一段临时脚本遍历 `rngSeed` 0..N，对**固定造态**（同一 carrier、同一 conceivedAt、同一生产月日历）调用 `buildBirth`，记录首个产出 `safe` 与首个 `child_dies` 的种子，填入下方测试的 `SAFE_SEED`/`CHILD_DIES_SEED`，删除脚本。确保造态字段全部固定，使裁决可复现。

- [ ] **Step 1: 写失败测试（固定 seed）**
...
const SAFE_SEED = 0;        // TODO 实现期填：使该胎息 bearerOutcome==="safe"
...
describe("生产健康成本（固定 seed，无条件断言）", () => {
  ...
  it("顺产成本致死（safe，health=5 → −5=0）：皇嗣存活，母亡", () => {
  ...
  it("难产 child_dies：母方 −10、无存活皇嗣；health=10 → 母方因成本死亡", () => {
    const { s, cid } = dueBirth(CHILD_DIES_SEED, 10);
    const plan = buildBirth(db, s)!;
    expect(plan.bearerOutcome).toBe("child_dies"); // 无条件
    const r = applyEffects(db, s, plan.effects);
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(r.value.resources.bloodline.heirs.some((h) => h.lifecycle === "alive")).toBe(false); // 子亡
    expect(r.value.standing[cid]!.lifecycle).toBe("deceased"); // 10 − 10 = 0 → 母亡
  });
});
```

> 造态字段名（`pregnancy`/`gestations`/`lifecycle === "carrying"`）须与 schema/既有 birth 测试一致；若仓库已有 birth 造态 helper，复用之，仅覆盖 `rngSeed` 与 `health`。
...
`src/store/gestation.ts` `buildBirth` 的 return：把单 `birth` 效果改为「birth + 母方成本」：
...
import：`import { childbirthCostDelta } from "./pregnancyCost"; import { planHealthChange } from "./health";`（`birthEffect`/`now`/`verdict`/`gest`/返回结构其它字段沿用现有局部变量名，按实际对齐）。

> `planHealthChange` 读**生产前** state（母方仍 carrying、未亡），`isDeceased` 守卫不误杀；`set_consort_health` apply 排在 birth 之后，若成本致 0 则其内部 `consort_decease` 置母方 deceased——皇嗣已由前 birth 效果落库（已产存嗣仅母亡）。

- [ ] **Step 4: 跑测试 + 类型 + 构建**

Run: `npx vitest run tests/store/birthCost.test.ts && npx tsc --noEmit && npx vite build`
Expected: PASS（3 测试），tsc 无错，build OK。
...
5. PhysicianModal 互斥歧义 → **T6** 唯一结构（看诊区常驻 + 流胎/承养共存）。✓
6. 文案显示实际回血 → **T5** `actualHealing` + **T6** 文案规则（0 回血不显示虚假 +N）。✓
7. 生产空断言 → **T9** 固定 `SAFE_SEED`/`CHILD_DIES_SEED`，无条件 `expect(bearerOutcome).toBe(...)`。✓

**3. Placeholder scan：** 仅 T9 留 `SAFE_SEED`/`CHILD_DIES_SEED` 待扫描——TDD 内禀步骤（Step 0 明确扫描方法），非计划缺口。✓

**4. Type consistency：** `HealthSubject` = `PhysicianSubject`；`record_physician_visit.subject` 判别联合在 T1 schema/validate/apply、T5、T6 一致；`PhysicianVisitPlan`（rolledHealing/actualHealing/cured）T5 定义、T6 消费一致；`childbirthCostDelta`（T8）被 T9 消费一致；`GateResult`（T10）被 T11 消费一致；`ConsultOption`（T5）与 PhysicianModal `consults` props 一致。✓

**实现期需对齐项**：① `pregnancy_transfer` 字段名以 schema 为准（T8）；② `GameStore` 构造/`resolveTimedAction` 命令形状与返回结构（T6）；③ `buildBirth` 造态用 `plannedBirthOf` 校准日历 + seed 扫描（T9）；④ `resolveTimedAction` 返回的 `healthOutcome.sovereignDied`/`rolledOver` 字段名（T6）；⑤ `pickSurname`/`pickGivenName`/`livingConsortIds` 签名（T3/T5）。
