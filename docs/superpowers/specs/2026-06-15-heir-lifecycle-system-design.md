...
// standing 另增 recoverUntilMonth?: number —— 产后休养（虚弱）截止月序，见 §3.7
```
...
### 3.5 生产（提前 / 临盆）
生产月：
- **自孕（carrier==="sovereign"）**：固定**孕十月**临盆，帝王不难产。
- **承嗣君（carrier=侍君）**：**孕八月、孕九月**各有一次**提前生产**几率（`earlyBirth` 配置，确定性 hash）；任一命中即当月生产，否则**孕十月**临盆。提前生产**等同正常生产**（难产判定、结局、休养完全一致）。
当确定的生产月到达：当月一个**确定性随机行动点** slot = `hash(seed, monthOrdinal(now)) % apMax`；玩家行动到该 slot（或更晚）触发一次 `birth`。**兜底**：若已过生产月仍未触发，下次任意行动立即补触发，绝不漏。
- **自孕**：安产；`fatherId=null`；嫡。
...
### 3.7 产后休养（虚弱）
生产后**存活**的承嗣君进入**虚弱**：期间召其侍寝**激情不可选**（仅享受/陪伴），到期恢复激情。按日历月计，记 `standing.recoverUntilMonth`（monthOrdinal）：
- **安产**（育嗣君）：虚弱 1 个月（生产次月虚弱，再次月恢复）→ `recoverUntilMonth = monthOrdinal(生产月) + 2`。
- **难产存活**（子嗣夭折）：虚弱 3 个月 → `recoverUntilMonth = monthOrdinal(生产月) + 4`。
- 承嗣君亡 / 一尸两命：`deceased`，无休养。帝王自孕不涉休养（非侍寝对象）。
- 月数 `safeMonths/dystociaMonths` 由 `world.gestation.recovery` 配置。
...
- **普通在世侍君**：激情 / 享受（不变）。
- **承嗣君（lifecycle==="carrying"）或休养中（`recoverUntilMonth` 未到）**：**激情不可选**，仅 **享受 / 陪伴**；两者都计入侍寝次数（受宠频率），都不受孕。
- **deceased 侍君**：不可侍寝（从入口排除）。
- 育嗣君（delivered，休养期满）/ candidate：按普通侍君（candidate 仍可激情；受孕已被 §3.6 单线封住）。

激情门控统一判据：`lifecycle==="carrying"` 或（`recoverUntilMonth!=undefined && monthOrdinal(now) < recoverUntilMonth`）→ 激情禁用，呈现「享受 / 陪伴」。
...
- **宠爱初值**：
  - 自孕：**100**（固定）。
  - 侍君承嗣：按**生产当月生父（承载侍君）的受宠程度**映射到 0–50 —— 盛宠 50 / 宠爱 38 / 小宠 25 / 失宠 12 / 无宠 0（`tierValues` 可配）；若 `bearer===凤后` 再 **+30**（凤后封顶 80）。
  - 受宠程度由 `computeFavorStats`（侍寝频率派生）取，不读数值 favor。
...
生产裁决后：
- **非凤后侍君安产**：凤后前来道贺「恭喜陛下喜得凤儿（皇子）/ 皇郎」+「是否晋升立功侍君（育嗣君）？」→ 复用位分选择器；侍君 `lifecycle:"delivered"`、`recoverUntilMonth=生产月+2`（虚弱 1 月，§3.7）。
- **凤后承嗣安产 / 自孕安产**：仅道贺通知，不弹晋升（凤后不可调位分；自孕无侍君）。凤后安产同样 `delivered` + 休养。
- **子嗣夭折**（难产存活）：无子嗣落地，侍君 `lifecycle:"normal"`、`recoverUntilMonth=生产月+4`（虚弱 3 月）；太医禀报。
- **承嗣君亡 / 一尸两命**：该侍君 `lifecycle:"deceased"`；宗正寺/太医禀报凶讯。
...
- 提前生产：承嗣君孕八月、孕九月各按 `earlyBirth.month8/month9` 几率（确定性 hash）判定，命中即当月生产。
- 休养：安产 `recovery.safeMonths`、难产存活 `recovery.dystociaMonths`（月）。
- 子嗣宠爱初值映射 `childFavor.tierValues`（受宠程度→0–50）+ 凤后 `fenghouBonus` / 自孕 `selfPregnancy`。
- 生产 slot、提前生产、性别、难产判定均经 `fnv1a64Hex` 哈希（输入含 rngSeed + monthOrdinal/dayIndex + 标识），存档/重放稳定。缺省走引擎内置 fallback。
...
**引擎纯逻辑（新建）**
- `src/engine/characters/gestation.ts` — `gestationMonth(now, conceivedAt)`、`dystociaChance(atMonth, cfg)`、提前生产判定（孕八/九月）、生产 slot、休养截止月计算。
- `src/engine/characters/birth.ts` — 生产裁决（难产结局/性别/宠爱初值，纯函数；宠爱读受宠程度 tier→tierValues）。
...
- `gestation`：孕月数锚点（受孕月=1，检出=2，传嗣=3，临盆=10）；难产几率曲线（孕三月 5%、孕九月上限）；生产 slot 确定性；提前生产判定（孕八/九月）确定性；休养截止月（安产 +2 / 难产存活 +4）。
- `birth`：结局占比分配确定性；性别确定性；宠爱初值（自孕 100 / 受宠程度 tier→0–50 / 凤后 +30）；嫡判定（凤后 or 自孕）。
...
- `bedchamber`：承嗣君 / 休养中仅享受/陪伴；陪伴计数不受孕；单线孕育封受孕；休养期满恢复激情。
