...
> 设计日期：2026-06-28
> 状态：已确认设计（3 轮评审通过），进入 writing-plans
...
`AdoptionRecord` / `RoyalResidence` 类型保持克制：只保存稳定事实身份，**不**提前塞入审批流状态机、府中经济、家眷状态或谋反指标。`RoyalResidence` **不持久化**法统子女列表；子女关系统一通过 `getLegalChildren` 从 `state.parentage` 派生（§4）。
...
不是普通 setter。若 `state.parentage[childId]` 已存在，**返回** `PARENTAGE_ALREADY_ESTABLISHED` domain error（经 `Result<GameState, GameError[]>` 通道，见 §6），**不**抛未捕获异常，且**不**修改输入 state。修复坏数据走迁移/开发工具，**不**走运行时命令。
...
迁移后强不变量：`∀ heir ∈ state.resources.bloodline.heirs, state.parentage[heir.id] != null`。缺 parentage 的皇嗣**不能**通过 state validation。正式 selector **禁止** `parentage?.biologicalFatherId ?? heir.fatherId` 这类静默 fallback（只能存在于迁移函数内部）。
...
- 引擎/存档：`src/engine/effects/funnel.ts`、`src/engine/content/schemas.ts`、`src/engine/save/saveSystem.ts`、`src/engine/save/stateSchema.ts`、`src/engine/state/types.ts`、`src/engine/characters/companionReconciliation.ts`、`src/engine/characters/custodianAvailability.ts`（line 32 `heir.adoptiveFatherId`，驱动月度成长结算的 `neglect`/`custodianBond`）
...
**月度成长结算回归**：`resolveCustodianAvailability`（`custodianAvailability.ts`）改读 `getCurrentCustodian` 后，`heirUpbringingSettlement.ts` 的月度 `neglect`/`custodianBond` 行为必须**保持不变**；须新增月度成长结算的 rename 回归测试。

> 注：`main` 推进持续改变此清单——`companionReconciliation.ts`、`custodianAvailability.ts` 均在首轮 sweep 后才并入 `main`。故实现时**以重跑 grep 为准**（当前 13 个 prod 文件），不可信赖本清单。
...
biological 图无环
legal 图无环
每个 mother/father 引用 ∈ knownPersonIds（见下）
```

**activeAdoptionRecordId 双向交叉不变量**（即使正常流程产生空 `adoptionRecords`，载入的旧档也不得放任悬空/矛盾记录）：
```text
# parentage → record（正向）
parentage.activeAdoptionRecordId 指向的 record 必须存在
record.childId === parentage key
record.status === "active"
record.newLegalMotherId === 当前 legalMotherId 且 record.newLegalFatherId === 当前 legalFatherId

# record → parentage（反向，闭合「同一 child 至多一条 active record」）
∀ status="active" 的 AdoptionRecord r:
  state.parentage[r.childId] 必须存在
  parentage[r.childId].activeAdoptionRecordId === r.id
  parentage[r.childId].legalMotherId === r.newLegalMotherId
  parentage[r.childId].legalFatherId === r.newLegalFatherId
同一 child 至多一条 active record
```

**map key 自洽：** `adoptionRecords[k].id === k`；`royalResidences[k].id === k`。

**「已知人物」（`knownPersonIds`）定义**：
```ts
knownPersonIds =
  { SOVEREIGN_PERSON_ID }                       // 显式哨兵
  ∪ state.resources.bloodline.heirs[*].id
  ∪ Object.keys(state.standing)
  ∪ Object.keys(state.generatedConsorts)
  ∪ Object.keys(db.characters)                  // 仅 content cross-check 阶段可见
```
分层校验：纯 `gameStateSchema` **只**做结构校验（无 `ContentDB` 访问）；需要 `db` 的引用完整性（parent id ∈ `knownPersonIds`）放在加载流程的 **content-id cross-check** 阶段或接收 `db` 的 validator 中执行。`SOVEREIGN_PERSON_ID` 始终合法，**不**要求其存在于 `db.characters` / `state.standing`；validator **不得**要求所有父母都存在于 `state.standing`。
...
getLegalDescendants(state, parentId: PersonId, maxDepth?): CharacterId[]  // 根可为 SOVEREIGN_PERSON_ID；从某法统父母向下

getCurrentCustodian(state, childId): CharacterId | undefined         // 读 Heir.custodianId
```

**结果顺序稳定**：parents 由结构表达；`getBiological/LegalChildren` 按 `CharacterId` 升序；`getLegalDescendants` **按世代广度优先，同一世代按 `CharacterId` 升序**（不做跨世代全局排序，以免破坏谱系层级）；`getBiologicalAncestors` 按层级 + 母系优先的固定规则。
...
22. 悬空、错 child、非 active 的 `activeAdoptionRecordId`，**以及**未被 parentage 反向引用（或法统父母不一致）的 `status="active"` AdoptionRecord，均**不能**通过 validation。
