...
- 旧档迁移 `SAVE_FORMAT_VERSION` 13→14：缺失者按同口径回填，已存在的自定义值原样保留（不覆盖）。

## 二、年资

按**当前官职任职时间**计（`appointedAt` 已表「最近一次任职/调任」时刻）：`seniorityYears = max(0,
year − appointedAt.year)`；`seniorityScore` 归一化 0–100（`SENIORITY_FULL_YEARS=10` 年计满）。

## 三、家族势力 `familyBacking`（0–100，实时派生，不复制进官员字段）

```
consortScore  = rankScore*0.60 + favorScore*0.40   // rankScore=位分 order 归一化；favorScore=当前恩宠
...
只认明确 `familyId`/`birthFamilyId` 关联，**不靠同姓猜测亲缘**；排除 `deceased` 侍君；无侍君则
`consortBacking=0`。
