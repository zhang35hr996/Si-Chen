...
**Delta cap semantics (product rule, v0):** *a single effect batch cannot change the same numeric axis by more than 10 points in either direction* — one scene outcome can never cause an extreme state swing. Two +8s to the same axis landing as +10 total is by design, not a bug; content authors should know this. It is a **balancing rule, not a schema constraint**: the per-effect ±10 in the schema and the per-batch cumulative cap in the funnel are v0 defaults. When future authored content legitimately needs bigger moves (册封 rank-up, major plot blows), the answer is an explicit **privileged effect type** that the funnel validates on its own terms — never a bypass around the funnel.

**EffectReport is diagnostics, not world history:** `EffectReport` and the ring log are for developers; `GameState.eventLog` is what actually happened in the player's world. Rejected effect batches appear in diagnostics and NEVER in the eventLog — the event engine marks an event fired only after its entire resolution transaction commits.
...
| **7. Event engine + AP affordability** | Condition DSL evaluators, checkpoints wired into travel/rollover, once/priority/cooldown, affordability rule (engine-side, block entry + 「行动点不足」), event resolution as ONE transaction (funnel effects + AP spend + eventFired), interim "choose outcome" UI from flattened scene choices — replaced by PR 8's dialogue UI | `engine/events`, `ui` | All 3 events trigger at the right place/condition exactly once; unaffordable heavy event blocks with message, no auto-rollover; rejected effects never mark fired; DSL truth tables pass | Checkpoint wiring — integration tests |
