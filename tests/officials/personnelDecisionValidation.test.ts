...
/** Base state with the story consorts these tests reference (lu/wen/shen) injected,
 *  so their maternal families/officials (official_fam_lu_main, fam_wen_main, …) exist. */
const fresh = (): GameState =>
  ["lu_huaijin", "wenya", "shen_zhibai"].reduce((s, id) => withConsort(s, db, id), createNewGameState(db, 1));
...
describe("personnel decision validator — kind-specific invariants", () => {
  ...
  it("family implication source punishment targets a different consort", () => {
    const s = withConsortPun(fresh()); // pun_000001 targets LU_CONSORT
    const off = s.officials[LU_OFFICIAL]!;
    const d: PersonnelDecision = {
  ...
  it("family implication official belongs to a different family", () => {
    const s = withConsortPun(fresh()); // 受罚侍君 LU_CONSORT，母族 fam_lu_main
