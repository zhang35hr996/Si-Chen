...
describe("generated consort as bystander — memory funnel transaction", () => {
  ...
  it("planPunishmentConsequences produces memory for generated bystander without funnel rejection", () => {
    ...
    const withConsortResult = addGeneratedConsort(store.getState(), db, genContent, "cairen", 40);
    if (!withConsortResult.ok) throw new Error("addGeneratedConsort failed in test setup");
    store.loadState(withConsortResult.value);
  ...
  it("full batch is atomic — generated bystander with no memory store causes rollback", () => {
    ...
    const genContent = makeGeneratedConsort(bystanderId);
    const withConsortResult2 = addGeneratedConsort(store.getState(), db, genContent, "cairen", 40);
    if (!withConsortResult2.ok) throw new Error("addGeneratedConsort failed in test setup");
    const withConsort = withConsortResult2.value;
