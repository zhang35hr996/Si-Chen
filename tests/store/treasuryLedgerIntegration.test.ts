...
describe("P2-A: buyItem — 正价校验", () => {
  it("buyItem with negative price → false, treasury/ledger/inventory unchanged", () => {
    const store = storeWithTreasury(10_000);
    const beforeState = JSON.stringify(store.getState());
    const result = store.buyItem("luozidai", -100);
    expect(result).toBe(false);
    // State must be byte-identical (no treasury deduction, no item grant, no ledger entry)
    const afterState = JSON.stringify(store.getState());
    expect(afterState).toBe(beforeState);
  });

  it("buyItem with zero price → false, treasury/ledger/inventory unchanged", () => {
    const store = storeWithTreasury(10_000);
    const beforeState = JSON.stringify(store.getState());
    const result = store.buyItem("luozidai", 0);
    expect(result).toBe(false);
    expect(JSON.stringify(store.getState())).toBe(beforeState);
  });

  it("buyItem with non-integer price → false, treasury/ledger/inventory unchanged", () => {
    const store = storeWithTreasury(10_000);
    const beforeState = JSON.stringify(store.getState());
    const result = store.buyItem("luozidai", 99.99);
    expect(result).toBe(false);
    expect(JSON.stringify(store.getState())).toBe(beforeState);
  });

  it("buyItem with valid positive price succeeds and ledger entry is shop_purchase with negative delta", () => {
    const store = storeWithTreasury(10_000);
    const result = store.buyItem("luozidai", 200);
    expect(result).toBe(true);
    expect(store.getState().resources.nation.treasury).toBe(9_800);
    expect(store.getState().treasuryLedger).toHaveLength(1);
    expect(store.getState().treasuryLedger[0]!.delta).toBe(-200);
    expect(store.getState().treasuryLedger[0]!.source.kind).toBe("shop_purchase");
    expect(validateTreasuryLedger(store.getState())).toEqual([]);
  });
});
