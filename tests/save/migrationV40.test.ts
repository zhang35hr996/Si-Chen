/**
 * Save format v39 → v40 migration tests (PR4C-1：伴读关系身份与历任历史)。
 *
 * v40：active assignment 补稳定 id；遗留 ended map 条目迁入 endedCompanionAssignments；
 * 补 nextCompanionAssignmentSeq。结构重排，显式 normalize。
 * 存档从 formatVersion 38 起会先经宗亲 v38→v39，再经本迁移 v39→v40。
 */
...
describe("save migration → v40 (伴读关系身份与历任历史)", () => {
  it("V40-01: SAVE_FORMAT_VERSION >= 40", () => {
    expect(SAVE_FORMAT_VERSION).toBeGreaterThanOrEqual(40);
  });

  it("V40-02: 旧档 active assignment 无 id → 稳定补 legacy id", () => {
  ...
  it("V40-03: 旧档 ended map 条目迁入 endedCompanionAssignments，并离开 active", () => {
    const storage = createMemoryStorage();
    storage.set(`${SAVE_KEY_PREFIX}slot1`, makeV38Save());
    const loaded = readSlot(storage, db, "slot1");
    expect(loaded.ok).toBe(true);
  ...
  it("V40-04: nextCompanionAssignmentSeq 越过已分配数量，避免未来 id 冲突", () => {
    const storage = createMemoryStorage();
    storage.set(`${SAVE_KEY_PREFIX}slot1`, makeV38Save());
    const loaded = readSlot(storage, db, "slot1");
    expect(loaded.ok).toBe(true);
  ...
  it("V40-05: 单个 active 伴读、数字结尾 heirId 的旧档可迁移并通过校验", () => {
    const s = createNewGameState(db);
    s.resources.bloodline.heirs = [
