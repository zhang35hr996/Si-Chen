...
describe("SPEND_AP", () => {
  it("decrements AP without rollover when AP remains", () => {
    ...
    expect(r.value.state.calendar.ap).toBe(4);
  ...
  it("rolls the action-day and refills AP when AP hits exactly 0", () => {
    const r = expectOk(applyCommand(freshState(), { type: "SPEND_AP", amount: 6 }));
  ...
  it("chains rollovers across 旬→月→年 via successive spends", () => {
    // 36 action-days in a year; spend a full 6-AP day each from 元年一月上旬.
    let state = freshState();
    for (let day = 0; day < 36; day++) {
      state = expectOk(applyCommand(state, { type: "SPEND_AP", amount: 6 })).value.state;
    }
    expect(state.calendar).toMatchObject({ year: 2, month: 1, period: "early", ap: 6 });
  });
  ...
  it("rejects overspend (AP_INSUFFICIENT) and leaves state untouched", () => {
    ...
    const r = expectErr(applyCommand(spent, { type: "SPEND_AP", amount: 4 }));
    expect(r.error.code).toBe("AP_INSUFFICIENT");
    expect(spent.calendar.ap).toBe(3); // untouched — AP can never go negative
  });
  ...
  it("never mutates the input state", () => {
    ...
    applyCommand(before, { type: "SPEND_AP", amount: 5 });
...
describe("applyBatch atomicity", () => {
  it("applies all commands and aggregates rolledOver", () => {
    const r = expectOk(
      applyBatch(freshState(), [
        ...
        { type: "SPEND_AP", amount: 6 }, // triggers rollover
  ...
  it("rejects the WHOLE batch when any command fails — earlier commands do not land", () => {
    ...
    expect(before.calendar.ap).toBe(6);
  });
